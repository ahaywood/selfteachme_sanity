    ...
    <Link href={`/blog/${content.slug.current}`}>
      <a className={className}>
        <div className="relative flex h-64">
          <img className="w-full h-full object-cover absolute top-0 left-0 gap-4" src={content.hero} />
          <div className="bg-white text-center relative m-2 px-2 py-6 w-full mt-auto">
            <h4 className="font-condensed uppercase text-baliHai text-lg md:text-2xl">
              {content?.postDetails?.datePublished && getPostMonth(content.postDetails.datePublished)}&nbsp;
              {content?.postDetails?.datePublished && getPostDate(content.postDetails.datePublished)}
            </h4>
